...
pub struct SlabVectorAllocator {
    list_table:[LinkedList;24]
    
}
impl VectorAllocator for SlabVectorAllocator {
    fn new(sta:usize ,capacity: usize) -> Self {
        unsafe{let mut ta = [LinkedList{head: ptr::null_mut() as *mut usize };24];
        for i in 0..1{
            ta[23].push((sta+i*8388608) as *mut usize);
        }
        Self {
            list_table : ta,
        }}
    ...
    fn alloc(&mut self, mut size: usize, align: usize) -> Option<usize> {
        ...
        //println!(" return {:0x}",t.unwrap() as usize );
        return t;
    }
    fn dealloc(&mut self, start: usize, mut size: usize, _align: usize) {
        ...
        let mut len =8388608;let mut p=23;
