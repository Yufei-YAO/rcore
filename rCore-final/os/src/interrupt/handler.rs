...
pub fn handle_interrupt(context: &mut Context, scause: Scause, stval: usize) -> *mut Context {
    match scause.cause() {
        ...
        Trap::Exception(Exception::StorePageFault) => pagefault_handler(context,stval),
        Trap::Exception(Exception::LoadPageFault) => pagefault_handler(context,stval),
        Trap::Exception(Exception::InstructionPageFault) => pagefault_handler(context,stval),
...
fn pagefault_handler(context: &mut Context, stval: usize)-> *mut Context{
    unsafe{sie::clear_stimer();}

    println!("page_fault_handler miss vp : {:0x}",stval>>12 );

    let mut m = FRAME_ALLOCATOR.lock().alloc();
    let mut frame:FrameTracker;
    if m.is_err(){
        FRAME_REPLACER.lock().alloc_fault();
        frame = FRAME_ALLOCATOR.lock().alloc().unwrap();
    }else{ 
        frame = m.unwrap();
    }
    //println!("page free ok");
    FRAME_REPLACER.lock().page_invaild(PROCESSOR.get().current_thread().process.clone(),VirtualAddress(stval),frame);
    //println!("page free ok2");
    unsafe{sie::set_stimer();}
    tick();
    context
}
